...
@section('title', 'Crear Inscripción Equipo')
...
<div class="p-6">
    <h1 class="text-2xl font-bold mb-4">{{ isset($inscripcion_equipo) ? 'Editar Inscripción Equipo' : 'Nueva Inscripción Equipo' }}</h1>

    <form id="formInscripcionEquipo" action="{{ isset($inscripcion_equipo) ? route('inscripcion_equipo.update', $inscripcion_equipo) : route('inscripcion_equipo.store') }}" method="POST" novalidate>
        @csrf
        @if(isset($inscripcion_equipo))
            @method('PUT')
        @endif
        ...
        <div class="form-group">
            ...
            <input type="date" name="fecha_inscripcion" id="fecha_inscripcion" value="{{ old('fecha_inscripcion', $inscripcion_equipo->fecha_inscripcion ?? '') }}">
            <span class="error-message"></span>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="estado" id="estado">
                ...
                <option value="Pendiente" {{ old('estado', $inscripcion_equipo->estado ?? '') == 'Pendiente' ? 'selected' : '' }}>Pendiente</option>
                <option value="Completado" {{ old('estado', $inscripcion_equipo->estado ?? '') == 'Completado' ? 'selected' : '' }}>Completado</option>
            </select>
            <span class="error-message"></span>
        </div>
